...
- Integración/contrato: `php artisan test --testsuite=Feature` — real y en verde desde S-00. La suite intenta abortar si la base configurada no termina en `_test`, pero **ese control es incompleto hoy**: QA demostró en la validación de S-00 que se elude por completo cuando la conexión llega por `DB_URL`, porque la salvaguarda examina el campo de configuración y no el nombre efectivo que resuelve el driver. Hasta que se corrija (QA-01, propietario `implementation-backend`), no se puede confiar en él: una corrida con `DB_URL` apuntando a la base de aplicación la destruiría con `migrate:fresh` y quedaría en verde. Importa antes de S-DO-02, que es donde el CI empieza a inyectar la conexión por esa vía
