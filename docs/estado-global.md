...
current_rfc_batch: [S-06-F, S-09-B, S-QA-01, S-DO-02]
...
- Roadmap del horizonte redactado con 19 sprints, en revisión del usuario.

## Bloqueantes
- Los RFC de cada sprint todavía no están redactados ni aprobados; ningún sprint puede pasar a `Planificación: LISTO` hasta entonces.
- Dos valores siguen `propuesto` y bloquean a los sprints que dependen de ellos: el tope de importe para boleta sin documento del cliente (S/ 700, bloquea S-05-B) y `docs/frontend/experiencia.md`, pendiente de aprobación del usuario (bloquea todos los sprints de frontend).

## Siguiente fase habilitada
- S-00 (fundación técnica), una vez aprobado su RFC. Ningún sprint tiene `Ejecución: LISTO` todavía.
