...
- **El idioma del mensaje es responsabilidad de la configuración, no de cada pantalla.** El sistema muestra los mensajes genéricos del validador y hasta el 2026-08-20 salían **en inglés**: `config/app.php` tenía `'locale' => 'en'` y no existía directorio `lang/`, así que un código de producto mal escrito mostraba *"The codigo field format is invalid."* a la persona que opera la caja. Era divergencia contra `docs/frontend/experiencia.md`, que declara que el texto mostrado es el mensaje en español.

  Se resuelve traduciendo en un solo lugar y **no** dejando que cada pantalla ponga su propio texto: eso crearía un segundo sitio donde vive el mensaje, que es el patrón que este proyecto lleva ocho instancias corrigiendo. Una pantalla que recibe un mensaje pobre lo reporta; no lo tapa.

  **Traducir no basta por sí solo, y esa es la parte que no estaba a la vista.** Los mensajes por defecto nombran el campo por su identificador técnico, así que traducidos siguen diciendo `codigo`, `numero_documento`, `precio_menor` — jerga técnica, que esta misma sección prohíbe. Hace falta además un mapa de identificador a nombre legible. **Ese mapa es una novena instancia del patrón de dos fuentes** —hay que mantenerlo igual a los nombres reales de los campos— y se cierra igual que la matriz de permisos: con una prueba que compare los dos y falle cuando divergen. Sin esa prueba, un campo nuevo aparece en pantalla con su nombre técnico y nada falla.

  **Y la comprobación de que quedó bien se hace en la pantalla, no en el archivo.** Un archivo de traducciones puede existir y no estar aplicándose, y desde el lado del código las dos cosas se ven igual: hay que recorrer los rechazos de una pantalla real y mirar el texto que le queda al usuario. Lo observó `qa`, que fue así como encontró el problema.
